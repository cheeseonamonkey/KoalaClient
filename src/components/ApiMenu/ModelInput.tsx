...
export const ModelInput: React.FC<ModelInputProps> = ({
  ...
  return (
    <div
      key={`model${index}`}
      className='mb-4 p-1 pb-4 border-b border-gray-600/50 shadow-inner'
    >
          ...
          >
            <span className='inline-block truncate max-w-full'>
              {_modelDef.endpoint !== undefined && apiAuth[_modelDef.endpoint]
                ? apiAuth[_modelDef.endpoint].endpoint.replace(
                    /^https?:\/\//,
                    ''
                  )
                : 'Endpoint Undefined'}
